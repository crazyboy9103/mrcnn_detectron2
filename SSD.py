...
class CustomDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        try:
            ...
            print("Error loading dataset, pass")     
            self.dir = data_dir
...
class Model(Module):
    def __init__(self, num_classes, device, parallel = False, batch_size=8):
        ...
        for file in os.listdir():
            if file.endswith(".pt") and "ssd" in file:
    ...
    def load(self, path):
        ...
        params = [p for p in self.model.parameters() if p.requires_grad]
        self.optimizer = torch.optim.Adam(params, lr=0.001 * (0.95 ** self.start_epoch))
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

        for param_group in self.optimizer.param_groups:
            param_group['lr'] = 0.001 * (0.95 ** self.start_epoch)

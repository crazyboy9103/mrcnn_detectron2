...
class CustomDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        try:
            self.load()
            print("Data loaded from dataset_1224.pt")
            self.transform = transform # override transform to apply augmentation
        except:
            print("Error loading dataset_1224.pt, pass")
    ...
    def save(self):
        ...
        torch.save(temp, "dataset_1224.pt")
        print("dataset saved to dataset_1224.pt")

    def load(self):
        temp = torch.load("dataset_1224.pt")
        self.dir = temp["dir"]
...
def collate_fn(batch):
    ...
    return imgs, targets
class Model(Module):
    def __init__(self, num_classes, device, parallel = False, batch_size=8):
        Module.__init__(self)
        self.batch_size=batch_size
        self.num_classes=num_classes
        self.model = self.build_model(num_classes, device, parallel)
        params = [p for p in self.model.parameters() if p.requires_grad]
        self.optimizer = torch.optim.Adam(params, weight_decay=1e-4, lr=0.0001)
        ...
        for file in os.listdir():
            if file.endswith(".pt") and "mrcnn" in file:
    ...
    def build_model(self, num_classes, device, parallel):
        ...
        anchor_generator = AnchorGenerator(sizes = ((8, 16, 32, 64, 128, 256, 512),), aspect_ratios=((0.5, 1.0, 2.0), ))
    ...
    def fit(self, dataset, max_epochs):
        ...
        testloader = DataLoader(dataset = testset, batch_size=1, shuffle=False, num_workers=4,collate_fn=collate_fn)
    ...
    def load(self, path):
        ...
        for param_group in self.optimizer.param_groups:
            param_group['lr'] = 0.0001 * (0.95 ** checkpoint['epoch'])
